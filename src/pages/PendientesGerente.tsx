...
export default function PendientesGerente() {
  ...
  const fetchPendientes = async () => {
    try {
      ...
      if (data.ok && Array.isArray(data.data)) {
        // Filtrar solo pendientes con prefijo PENI- (Gerente)
        const gerentePendientes = data.data.filter((p: Pendiente) => p.id.startsWith('PENI-'));
        
        // Filtrar completadas de días anteriores
        const hoy = new Date().toISOString().split('T')[0];
        const filtradas = gerentePendientes.filter((p: Pendiente) => {
    ...
    try {
      const response = await fetch(PENDIENTES_API_URL, {
        ...
        body: JSON.stringify({
          ...
          action: 'pendientes.create.gerente', // Acción especial para Gerente
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Pendientes Gerente</h1>
